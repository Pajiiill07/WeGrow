...
<div class="content">
    ...
    <form action="{{ route('order-backend.store') }}" method="post" class="form">
        @csrf
        <h3>Tambahkan Data Pesanan</h3>
        <div>
        ...
        <div>
            ...
            <input type="text" class=" @error('quantity') is-invalid @enderror" name="quantity" value="{{ old('quantity')}}">
            @error('quantity')
            <div>
                {{ $message }}
            </div>
