...
<div class="content">
    ...
    <form action="{{ route('reserve-backend.store') }}" method="post" class="form">
        @csrf
        <h3>Tambahkan Data Reservasi</h3>
        <div>
            <label for="reserve_date">Tanggal Reservasi</label>
            <input type="text" class=" @error('reserve_date') is-invalid @enderror" name="reserve_date" value="{{ old('reserve_date')}}">
            @error('reserve_date')
            <div>
                {{ $message }}
            </div>
        ...
        <div>
            ...
            <input type="text" maxlength="50" class=" @error('reserve_time') is-invalid @enderror" name="reserve_time" value="{{ old('reserve_time')}}">
            @error('reserve_time')
            <div>
                {{ $message }}
            </div>
        ...
        <div>
            ...
            <input type="text" class=" @error('num_visitors') is-invalid @enderror" name="num_visitors" value="{{ old('num_visitors')}}">
            @error('num_visitors')
            <div>
                {{ $message }}
            </div>
        ...
        <div>
            ...
            <select class=" @error('meja_id') is-invalid @enderror" name="meja_id">
                ...
                <option value="{{$meja->id}}" selected>{{$meja->meja_name}}</option>
                @else
